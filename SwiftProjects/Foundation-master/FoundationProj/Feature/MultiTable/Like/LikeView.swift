...
class LikeView: UIBasePreviewTypeForSampling {
    ...
    lazy var tableView = UITableView().then {
        ...
        $0.rowHeight = 92
        $0.backgroundView = ARTableViewEmptyView(type: .like)
//        $0.tableHeaderView = topView
        $0.register(LikeTableViewCell.self, forCellReuseIdentifier: LikeTableViewCell.reuseIdentifier())
    }
...
//        networkErrorView.bindingViewModel()
    ...
    // 좋아요 데이터
    func setupDI(observable: BehaviorRelay<[RecentlyLikeList]>) {
        // model Dependency Injection
        observable.asObservable().bind(to: tableView.rx.items(cellIdentifier: LikeTableViewCell.reuseIdentifier(), cellType: LikeTableViewCell.self)) { [weak self] _, element, cell in
            guard let `self` = self else { return }
        ...
        tableView
            ...
            .bind(to: showDetailTrigger).disposed(by: rx.disposeBag)
...
extension LikeView: UITableViewDelegate {
