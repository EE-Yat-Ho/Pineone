...
class ReplyView: UIBasePreviewTypeForSampling {
    // MARK: - Model type implemente
    typealias Model = RecentlyLikeList

//    let refreshTrigger = PublishRelay<Void>()
//    let showDetailTrigger = PublishRelay<ReplyList>()
...
//        $0.estimatedRowHeight = 96
//        $0.separatorStyle = .none
//        $0.rowHeight = UITableView.automaticDimension
////        $0.tableHeaderView = topView
//        $0.backgroundView = ARTableViewEmptyView(type: .reply)
//        $0.register(ReplyTableViewCell.self, forCellReuseIdentifier: ReplyTableViewCell.reuseIdentifier())
//    }
//
//    // 상단 새로고침 컨트롤
...
//
//    // 댓글 데이터
//    func setupDI(observable: BehaviorRelay<[ReplyList]>) {
//        // model Dependency Injection
//        observable.asObservable().bind(to: tableView.rx.items(cellIdentifier: ReplyTableViewCell.reuseIdentifier(), cellType: ReplyTableViewCell.self)) { [weak self] _, element, cell in
//            guard let `self` = self else { return }
...
//                .bind(to: r)
//                .disposed(by: rx.disposeBag)
//        } else if let r = relay as? PublishRelay<ReplyList> {
...
//    }
//
//    /// 삭제모드아닐때 셀클릭시 셀렉트 효과 지우기 위한 함수
//    func deSelectRowAll(observable: PublishRelay<Void>) {
//        observable
//            .map { _ in false }
//            .on(next: tableView.deSelectRowAll(animated:))
//            .disposed(by: rx.disposeBag)
//    }
}
//
...
//}
//
//extension ReplyView: UITableViewDelegate {
...
@available(iOS 13.0, *)
struct Reply_Previews: PreviewProvider {
